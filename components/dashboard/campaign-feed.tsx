  ...
  return (
    <div className="bg-card border border-border">
      <div className="p-4 border-b border-border flex items-center justify-between">
        <h2 className="text-sm font-medium">Feed de Operações</h2>
        <span className="text-xs text-muted-foreground font-mono">
          {campaigns.filter((c) => c.status === "live").length} ativas
        </span>
      </div>
  ...
  return (
    ...
    >
      ...
      <div className="w-24 text-right shrink-0">
        <p className="text-sm font-mono">R$ {campaign.spend.toLocaleString()}</p>
          ...
          {canToggle && (
            <DropdownMenuItem onClick={() => toggleCampaignStatus(campaign.id)}>
              {campaign.status === "live" ? (
                <><Pause className="w-4 h-4 mr-2" /> Pausar Operação</>
              ) : (
                <><Play className="w-4 h-4 mr-2" /> Retomar Operação</>
              )}
            </DropdownMenuItem>
          )}
          <DropdownMenuItem
  ...
  return (
    <div className="bg-card border border-border p-6">
      <div className="flex items-center justify-between mb-4">
        <h2 className="text-sm font-medium">Orçamento Diário</h2>
        <span className="text-xs text-muted-foreground font-mono">
